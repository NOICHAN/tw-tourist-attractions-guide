<template>
   <div class="contentMain">
    <div class="row flex-column flex-sm-row">
      ...
      v-for="item in sightseeingList" :key="item.ActivityID">
        <div class="card flex-row flex-sm-column">
          <img :src="item.Picture.PictureUrl1"
          class="card-img-top" :alt="item.ActivityName">
          <button type="button" class="favorite bg-white border-0"><span></span></button>
          <div class="card-body d-flex flex-column justify-content-between">
            <h5 class="card-title text-secondary">{{ item.ActivityName }}</h5>
            <div class="d-flex">
...
watchEffect(() => {
  ...
  console.log('Activity', sightseeingList.value);
});
